#ifndef DYNAMIXEL_STATUSRETURN_HPP
#define DYNAMIXEL_STATUSRETURN_HPP
...
namespace dynamixel {
    ...
    struct StatusReturnCommand {
        ...
        StatusReturnCommand(uint8_t id, const CommandError& error, const std::vector<uint8_t>& data, uint16_t crc)
            : magic(0x00FDFFFF)
            , id(id)
            , length(N + 4)
...
#endif  // DYNAMIXEL_STATUSRETURN_HPP
